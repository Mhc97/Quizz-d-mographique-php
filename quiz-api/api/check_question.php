...
if (!isset($input['questionId']) || !isset($input['selectionOption'])){
    echo json_encode(["correct" => false, "error" => "Données manquantes"]);
}

try {
    $pdo = new PDO('mysql:host=localhost;dname=pays;charset=utd8','root','');
    $stmt = $pdo->prepare('SELECR capital FROM pays WHERE id = ?');
